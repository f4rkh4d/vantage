...
final class SystemMonitorManager: ObservableObject {
    ...
    private func refreshProcesses() {
        Task.detached { [weak self] in
            ...
            let sorted = entries.sorted { $0.cpuPercent > $1.cpuPercent }.prefix(50).map { $0 }
            guard let manager = self else { return }
            await MainActor.run { manager.processes = sorted }
        }
    }
    ...
    private func refreshNetwork() {
        Task.detached { [weak self] in
            ...
            let capturedIn = totalIn
            let capturedOut = totalOut
            guard let manager = self else { return }
            await MainActor.run {
                if manager.lastNetIn > 0 {
                    manager.netInKBs  = Double(capturedIn  > manager.lastNetIn  ? capturedIn  - manager.lastNetIn  : 0) / 3000.0
                    manager.netOutKBs = Double(capturedOut > manager.lastNetOut ? capturedOut - manager.lastNetOut : 0) / 3000.0
                }
                manager.lastNetIn  = capturedIn
                manager.lastNetOut = capturedOut
            }
        }
    }
}
